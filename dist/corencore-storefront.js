(function() {
      ...
      <div class="container header-inner">
        ...
        <div class="header-actions">
          ...
          <select class="currency-select" id="currency-switcher">
            ${r.map(s=>`
              <option value="${s.code}" ${e.currency===s.code?"selected":""}>
                ${s.code} (${s.symbol})
              </option>
            `).join("")}
          </select>
          ...
          <button class="cart-trigger" id="cart-drawer-toggle-btn" aria-label="View Shopping Cart">
            <span>\u{1F6D2}</span>
            <span>Cart</span>
            <span class="cart-badge" id="cart-counter-badge">${o}</span>
          </button>
        </div>
      </div>
    </div>
  `,a.querySelector("#currency-switcher").addEventListener("change",s=>{i.setCurrency(s.target.value)}),a.querySelector("#cart-drawer-toggle-btn").addEventListener("click",()=>{i.toggleCart(!0)}),a.querySelector("#open-admin-hub-btn").addEventListener("click",()=>{i.openAdminHub()}),a.querySelector("#header-search-btn").addEventListener("click",()=>{let s=prompt("Search Mother & Baby products (e.g. belly butter, baby wash, lavender, diaper cream):");if(s!==null){i.searchProducts(s);let d=document.getElementById("product-grid-container");d&&d.scrollIntoView({behavior:"smooth"})}}),a.querySelectorAll("[data-filter]").forEach(s=>{s.addEventListener("click",d=>{d.preventDefault();let c=s.getAttribute("data-filter");i.setCategoryFilter(c);let n=document.getElementById("product-grid-container");n&&n.scrollIntoView({behavior:"smooth"})})}),a.querySelector("#logo-home-btn").addEventListener("click",s=>{s.preventDefault(),i.setCategoryFilter("all"),window.scrollTo({top:0,behavior:"smooth"})})}function $(a,e,i){a.innerHTML=`
    ...
    <div class="categories-section">
      <div class="container">
        ...
        <div class="categories-grid">
          ${i.map(r=>`
            <div class="category-card" data-category="${r.id}">
              <img src="${r.image}" alt="${r.name}" class="category-img" loading="lazy" />
              <div class="category-overlay">
                <span class="category-count">${r.count}</span>
                <h3 class="category-name">${r.name}</h3>
                <p style="font-size: 0.85rem; opacity: 0.9; margin-bottom: 12px;">${r.tagline}</p>
                <div class="category-action">${r.actionText}</div>
              </div>
            </div>
          `).join("")}
        </div>
  ...
  `,a.querySelectorAll(".category-card").forEach(r=>{r.addEventListener("click",()=>{let o=r.getAttribute("data-category");e.setCategoryFilter(o);let l=document.getElementById("product-grid-container");l&&l.scrollIntoView({behavior:"smooth"})})})}function F(a,e,i){let r=1,o={stage:"pregnant",concern:"stretch-marks",preference:"butter"},l=[{step:1,title:"Who are you shopping for today?",subtitle:"Select the stage to help us personalize safe, biocompatible ingredients.",options:[{key:"stage",value:"pregnant",title:"Expecting Mother",desc:"Trimesters 1, 2, or 3 (Prenatal Care)",icon:"\u{1F930}"},{key:"stage",value:"postpartum",title:"Postpartum Mother",desc:"4th Trimester recovery & breastfeeding",icon:"\u{1F931}"},{key:"stage",value:"newborn",title:"Newborn Infant",desc:"0 to 6 months delicate skin",icon:"\u{1F476}"},{key:"stage",value:"toddler",title:"Baby & Toddler",desc:"6 months+ active bath & play care",icon:"\u{1F37C}"}]},{step:2,title:"What is your main skin or care focus?",subtitle:"Our certified herbal formulas target specific developmental milestones.",options:[{key:"concern",value:"stretch-marks",title:"Stretch Marks & Deep Elasticity",desc:"Soothe tight, itchy stretching skin",icon:"\u2728"},{key:"concern",value:"nursing-comfort",title:"Nursing & Nipple Healing",desc:"100% Ingestion-safe, lanolin-free balm",icon:"\u{1F496}"},{key:"concern",value:"baby-eczema",title:"Baby Dryness & Eczema",desc:"Tear-free colloidal oat barrier soothing",icon:"\u{1F6C1}"},{key:"concern",value:"sleep-calm",title:"Bedtime Sleep & Colic Relief",desc:"Lavender calming massage and sleep mist",icon:"\u{1F319}"}]},{step:3,title:"What texture and finish do you prefer?",subtitle:"All products are 100% non-sticky and fast-absorbing.",options:[{key:"preference",value:"butter",title:"Rich Whipped Body Butters",desc:"Deep 24hr moisture lock",icon:"\u{1F9C8}"},{key:"preference",value:"oil",title:"Silky Cold-Pressed Oils",desc:"Fast penetration with radiant glow",icon:"\u{1F4A7}"},{key:"preference",value:"wash-lotion",title:"Gentle Foams & Daily Lotions",desc:"Lightweight everyday hydration",icon:"\u{1F9F4}"},{key:"preference",value:"full-bundle",title:"Complete Complete Care Routine",desc:"Head-to-toe bundle with 15% savings",icon:"\u{1F381}"}]}];function h(){return o.stage==="pregnant"||o.concern==="stretch-marks"?{title:"The Ultimate Prenatal Elasticity Routine",desc:"Designed to maximize skin flexibility and lock in cellular moisture from trimester 1 through postpartum.",products:[e.products.find(t=>t.id===0x38d7eccd55b1f)||e.products[0],e.products.find(t=>t.id===0x38d7ecccd8df1)||e.products[1],e.products.find(t=>t.id===0x38d7eccd55b1d)||e.products[5]]}:o.stage==="postpartum"||o.concern==="nursing-comfort"?{title:"The Fourth Trimester Mama Recovery Routine",desc:"Hospital-grade clean soothing formulas for nursing nipples, restorative firming, and perineal recovery.",products:[e.products.find(t=>t.id===0x38d7eccd55b1e)||e.products[2],e.products.find(t=>t.id===0x38d7eccd55b27)||e.products[3],e.products.find(t=>t.id===0x38d7eccd55b18)||e.products[17]]}:o.concern==="sleep-calm"?{title:"The Blissful Baby Sleep & Calming Ritual",desc:"Pediatrician-tested nighttime ritual with organic French Lavender to encourage deep, tranquil infant sleep.",products:[e.products.find(t=>t.id===0x38d7eccd55b20)||e.products[7],e.products.find(t=>t.id===0x38d7eccd55b28)||e.products[8],e.products.find(t=>t.id===0x38d7eccd55b23)||e.products[13]]}:{title:"The Pure Newborn Head-to-Toe Starter Routine",desc:"Ultra-mild, tear-free essentials to protect baby's delicate microbiome and natural moisture barrier.",products:[e.products.find(t=>t.id===0x38d7eccd55b20)||e.products[7],e.products.find(t=>t.id===0x38d7eccd55b26)||e.products[9],e.products.find(t=>t.id===0x38d7eccd55b25)||e.products[11]]}}function m(){if(r<=3){let t=l[r-1],s=r/3*100;a.innerHTML=`
          ...
          <div class="quiz-section">
            <div class="quiz-card">
              ...
              <div class="quiz-options-grid">
                ${t.options.map(n=>`
                    <div class="quiz-option ${o[n.key]===n.value?"selected":""}" data-key="${n.key}" data-value="${n.value}">
              ...
              <div class="quiz-navigation">
                ${r>1?'<button class="btn-secondary" id="quiz-prev-btn">\u2190 Back</button>':"<div></div>"}
                <button class="btn-primary" id="quiz-next-btn">
                  ${r===3?"Get My Personalized Routine \u2728":"Continue \u2192"}
                </button>
              </div>
            </div>
          </div>
        </div>
      `,a.querySelectorAll(".quiz-option").forEach(n=>{n.addEventListener("click",()=>{let p=n.getAttribute("data-key"),y=n.getAttribute("data-value");o[p]=y,m()})}),a.querySelector("#quiz-next-btn").addEventListener("click",()=>{r++,m()});let c=a.querySelector("#quiz-prev-btn");c&&c.addEventListener("click",()=>{r--,m()})}else{let t=h(),s=t.products.reduce((c,n)=>c+parseFloat((n==null?void 0:n.price)||0),0),d=(s*.85).toFixed(2);a.innerHTML=`
        <div class="container">
          <div class="quiz-section">
            <div class="quiz-card quiz-result-box">
              ...
              <div class="quiz-bundle-action">
                <div class="bundle-pricing">
                  ...
                  <span class="bundle-price-now">${i.formatPrice(d)}</span>
                  <span class="bundle-price-was">${i.formatPrice(s)}</span>
      ...
      `,a.querySelector("#quiz-retake-btn").addEventListener("click",()=>{r=1,m()}),a.querySelector("#quiz-add-bundle-btn").addEventListener("click",()=>{t.products.forEach(c=>{i.addToCart(c,1)}),i.toggleCart(!0)})}}m()}function v(a,e,i){let r=[{id:"all",label:"All Products (18)"},{id:"mother-care",label:"Mother Skin Care"},{id:"baby-care",label:"Baby Care"},{id:"bundles",label:"Bundles & Gift Sets"}],o=[...e.products];if(e.activeCategory&&e.activeCategory!=="all"&&(o=o.filter(t=>t.category===e.activeCategory)),e.searchQuery){let t=e.searchQuery.toLowerCase();o=o.filter(s=>s.title.toLowerCase().includes(t)||s.tags.toLowerCase().includes(t)||s.short_description.toLowerCase().includes(t))}e.sortBy==="price-low"?o.sort((t,s)=>parseFloat(t.price)-parseFloat(s.price)):e.sortBy==="price-high"?o.sort((t,s)=>parseFloat(s.price)-parseFloat(t.price)):e.sortBy==="rating"&&o.sort((t,s)=>s.rating-t.rating),a.innerHTML=`
      ...
      <div class="container">
        ...
        `:`
          <div class="products-grid">
            ${o.map(t=>{var c,n;let s=t.image||((c=t.images)==null?void 0:c[0]),d=((n=t.images)==null?void 0:n[1])||s;return`
  ...
  `,a.querySelectorAll(".filter-tab").forEach(t=>{t.addEventListener("click",()=>{let s=t.getAttribute("data-tab");i.setCategoryFilter(s)})});let l=a.querySelector("#product-sort-select");l&&l.addEventListener("change",t=>{i.setSortBy(t.target.value)});let h=a.querySelector("#clear-search-btn");h&&h.addEventListener("click",()=>{i.searchProducts("")});let m=a.querySelector("#reset-filter-btn");m&&m.addEventListener("click",()=>{i.setCategoryFilter("all"),i.searchProducts("")}),a.querySelectorAll("[data-quickview]").forEach(t=>{t.addEventListener("click",s=>{s.stopPropagation();let d=t.getAttribute("data-quickview"),c=e.products.find(n=>String(n.id)===String(d));c&&i.openProductModal(c)})}),a.querySelectorAll(".product-card").forEach(t=>{t.addEventListener("click",s=>{if(s.target.closest("[data-add-to-cart]"))return;let d=t.getAttribute("data-product-id"),c=e.products.find(n=>String(n.id)===String(d));c&&i.openProductModal(c)})}),a.querySelectorAll(".product-image-box").forEach(t=>{let s=t.querySelector(".product-img"),d=s.getAttribute("data-main"),c=s.getAttribute("data-hover");c&&c!==d&&(t.addEventListener("mouseenter",()=>{s.src=c}),t.addEventListener("mouseleave",()=>{s.src=d}))}),a.querySelectorAll("[data-add-to-cart]").forEach(t=>{t.addEventListener("click",s=>{s.stopPropagation();let d=t.getAttribute("data-add-to-cart"),c=e.products.find(n=>String(n.id)===String(d));c&&(i.addToCart(c,1),i.toggleCart(!0))})})}function C(a,e,i,r){var t,s;if(!e){a.innerHTML="";return}let o=((t=e.variants)==null?void 0:t[0])||{id:1,title:"Standard",price:e.price},l=1,h=e.image||((s=e.images)==null?void 0:s[0]);function m(){a.innerHTML=`
          ...
          <div class="quickview-grid">
            ...
            <div class="quickview-info">
              ...
              <div class="quickview-price-box">
                <span class="quickview-price">${r.formatPrice(o.price||e.price)}</span>
                ${e.compare_at_price?`
                  <span class="quickview-compare-price">${r.formatPrice(o.compare_at_price||e.compare_at_price)}</span>
                `:""}
              </div>
              ...
              ${e.variants&&e.variants.length>1?`
                <div class="variant-options-group">
                  ...
                  <div class="variant-chips">
                    ${e.variants.map(p=>`
                      <button 
                        class="variant-chip ${p.id===o.id?"selected":""}" 
                        data-variant-id="${p.id}"
                      >
                        ${p.title} \u2014 ${r.formatPrice(p.price)}
                      </button>
                    `).join("")}
                  </div>
                </div>
              ...
              <div style="display: flex; gap: 14px; align-items: center; margin: 24px 0;">
                <div class="cart-qty-stepper" style="padding: 4px;">
                  ...
                  <span class="qty-num" id="modal-qty-val">${l}</span>
                  <button class="qty-btn" id="modal-qty-plus">+</button>
                </div>

                <button class="btn-primary" id="modal-add-cart-btn" style="flex-grow: 1;">
                  <span>\u{1F6D2} Add to Cart \u2022 ${r.formatPrice((parseFloat(o.price||e.price)*l).toFixed(2))}</span>
                </button>
              </div>
              ...
              <div style="background: var(--bg-surface-secondary); padding: 16px; border-radius: var(--radius-md); margin-top: auto;">
                ...
                <div style="display: flex; flex-wrap: wrap; gap: 8px;">
                  ${(e.safety_badges||["Dermatologist Tested","100% Organic","Pediatrician Approved"]).map(p=>`
    ...
    `;let d=a.querySelector("#product-modal-overlay"),c=a.querySelector("#modal-close-btn"),n=()=>{r.closeProductModal()};c.addEventListener("click",n),d.addEventListener("click",p=>{p.target===d&&n()}),a.querySelectorAll(".thumb-img").forEach(p=>{p.addEventListener("click",()=>{h=p.getAttribute("data-src"),m()})}),a.querySelectorAll(".variant-chip").forEach(p=>{p.addEventListener("click",()=>{let y=parseInt(p.getAttribute("data-variant-id"));o=e.variants.find(S=>S.id===y)||o,m()})}),a.querySelector("#modal-qty-minus").addEventListener("click",()=>{l>1&&(l--,m())}),a.querySelector("#modal-qty-plus").addEventListener("click",()=>{l++,m()}),a.querySelector("#modal-add-cart-btn").addEventListener("click",()=>{r.addToCart(e,l,o),r.closeProductModal(),r.toggleCart(!0)})}m()}function b(a,e,i){let r=e.isCartOpen,o=e.cart.reduce((u,g)=>u+parseFloat(g.price)*g.quantity,0),l=0;e.appliedPromo==="MOMCARE15"?l=o*.15:e.appliedPromo==="PUREBABY"&&(l=Math.min(10,o));let h=Math.max(0,o-l),m=50,t=Math.min(100,o/m*100),s=Math.max(0,m-o);a.innerHTML=`
    <!-- Overlay -->
    <div class="cart-overlay ${r?"open":""}" id="cart-drawer-overlay"></div>

    <!-- Drawer -->
    <div class="cart-drawer ${r?"open":""}">
      <!-- Header -->
      ...
      <div class="free-shipping-tracker">
        <div class="free-shipping-text">
          ${s===0?"\u{1F389} You have unlocked <strong>FREE Standard Shipping!</strong>":`Add <strong>${i.formatPrice(s.toFixed(2))}</strong> more for <strong>FREE Shipping!</strong>`}
        </div>
            ...
            <div class="cart-item-details">
              ...
              <div class="cart-item-bottom">
                ...
                <div class="cart-item-price">
                  ${i.formatPrice((parseFloat(u.price)*u.quantity).toFixed(2))}
                </div>
              </div>
            </div>
      ...
      ${e.cart.length>0?`
        <div class="cart-footer">
          ...
          <div class="cart-summary-row">
            <span>Subtotal</span>
            <span>${i.formatPrice(o.toFixed(2))}</span>
          </div>

          ${l>0?`
            <div class="cart-summary-row" style="color: var(--color-accent); font-weight: 700;">
              <span>Promo Discount (${e.appliedPromo})</span>
              <span>-${i.formatPrice(l.toFixed(2))}</span>
            </div>
          `:""}

          <div class="cart-summary-row">
            <span>Estimated Shipping</span>
            <span>${s===0?"FREE":i.formatPrice("4.99")}</span>
          </div>

          <div class="cart-total-row">
            <span>Total</span>
            <span>${i.formatPrice((h+(s===0?0:4.99)).toFixed(2))}</span>
          </div>
  ...
  `;let d=a.querySelector("#cart-drawer-overlay"),c=a.querySelector("#cart-close-btn"),n=()=>i.toggleCart(!1);d&&d.addEventListener("click",n),c&&c.addEventListener("click",n);let p=a.querySelector("#cart-start-shopping-btn");p&&p.addEventListener("click",()=>{n();let u=document.getElementById("product-grid-container");u&&u.scrollIntoView({behavior:"smooth"})}),a.querySelectorAll("[data-cart-minus]").forEach(u=>{u.addEventListener("click",()=>{let g=u.getAttribute("data-cart-minus");i.updateCartQuantity(g,-1)})}),a.querySelectorAll("[data-cart-plus]").forEach(u=>{u.addEventListener("click",()=>{let g=u.getAttribute("data-cart-plus");i.updateCartQuantity(g,1)})}),a.querySelectorAll("[data-cart-remove]").forEach(u=>{u.addEventListener("click",()=>{let g=u.getAttribute("data-cart-remove");i.removeFromCart(g)})});let y=a.querySelector("#promo-apply-btn"),S=a.querySelector("#promo-code-input");y&&S&&y.addEventListener("click",()=>{let u=S.value.trim().toUpperCase();u==="MOMCARE15"||u==="PUREBABY"?i.applyPromo(u):u===""?i.applyPromo(""):alert("Invalid promo code. Try 'MOMCARE15' for 15% off!")});let x=a.querySelector("#cart-checkout-btn");x&&x.addEventListener("click",()=>{i.proceedToCheckout()})}function L(a){a.innerHTML=`
  ...
  `,a.querySelector("#guide-newsletter-form").addEventListener("submit",r=>{r.preventDefault(),alert("\u{1F389} Thank you! Your free Mother & Baby Care Routine Guide has been sent to your email."),r.target.reset()}),e.querySelectorAll("[data-footer-filter]").forEach(r=>{r.addEventListener("click",o=>{o.preventDefault();let l=r.getAttribute("data-footer-filter");i.setCategoryFilter(l);let h=document.getElementById("product-grid-container");h&&h.scrollIntoView({behavior:"smooth"})})}),e.querySelectorAll("[data-footer-nav]").forEach(r=>{r.addEventListener("click",o=>{o.preventDefault();let l=document.getElementById("routine-quiz-container");l&&l.scrollIntoView({behavior:"smooth"})})}),e.querySelectorAll("[data-page]").forEach(r=>{r.addEventListener("click",o=>{o.preventDefault();let l=r.getAttribute("data-page");i.openPageModal(l)})}),e.querySelector("#footer-admin-btn").addEventListener("click",()=>{i.openAdminHub()})}function _(a,e,i){if(!e){a.innerHTML="";return}let r={"routine-guide":{title:"Mother & Baby Care Routine Guide",subtitle:"Gentle, Biocompatible Daily Rituals for Mama and Child",content:`
      ...
      `}},o=r[e]||r["routine-guide"];a.innerHTML=`
  ...
  `;let l=a.querySelector("#page-modal-overlay"),h=a.querySelector("#page-modal-close-btn"),m=a.querySelector("#page-modal-done-btn"),t=()=>i.closePageModal();h&&h.addEventListener("click",t),m&&m.addEventListener("click",t),l&&l.addEventListener("click",s=>{s.target===l&&t()})}function B(a,e,i){if(!e.isAdminHubOpen){a.innerHTML="";return}a.innerHTML=`
  ...
  `;let r=a.querySelector("#admin-hub-overlay"),o=a.querySelector("#admin-hub-close-btn"),l=()=>i.closeAdminHub();o&&o.addEventListener("click",l),r&&r.addEventListener("click",n=>{n.target===r&&l()});let h=a.querySelector("#btn-sync-catalog-action"),m=a.querySelector("#sync-catalog-status");h.addEventListener("click",async()=>{h.disabled=!0,h.innerHTML="<span>\u23F3 Syncing to ShopBase...</span>",m.style.color="var(--color-primary)",m.textContent="Updating 18 products via Admin REST API...";try{let p=await(await fetch("/api/sync-catalog",{method:"POST"})).json();p.success?(m.style.color="#2E7D32",m.textContent=`\u2705 ${p.message}`,h.innerHTML="<span>\u2713 Synced Successfully</span>"):(m.style.color="#D32F2F",m.textContent=`\u274C ${p.error||"Sync failed"}`,h.innerHTML="<span>Retry Sync</span>",h.disabled=!1)}catch(n){m.style.color="#D32F2F",m.textContent=`\u274C Network Error: ${n.message}`,h.innerHTML="<span>Retry Sync</span>",h.disabled=!1}});let t=a.querySelector("#btn-sync-pages-action"),s=a.querySelector("#sync-pages-status");t.addEventListener("click",async()=>{t.disabled=!0,t.innerHTML="<span>\u23F3 Syncing Pages...</span>",s.style.color="var(--color-primary)",s.textContent="Publishing Routine Guide & Clean Safety Standards...";try{let p=await(await fetch("/api/sync-pages",{method:"POST"})).json();p.success?(s.style.color="#2E7D32",s.textContent="\u2705 Pages published to ShopBase store!",t.innerHTML="<span>\u2713 Pages Synced</span>"):(s.style.color="#D32F2F",s.textContent=`\u274C ${p.error||"Pages sync failed"}`,t.innerHTML="<span>Retry Sync</span>",t.disabled=!1)}catch(n){s.style.color="#D32F2F",s.textContent=`\u274C Network Error: ${n.message}`,t.innerHTML="<span>Retry Sync</span>",t.disabled=!1}});let d=a.querySelector("#btn-sync-homepage-action"),c=a.querySelector("#sync-homepage-status");d.addEventListener("click",async()=>{d.disabled=!0,d.innerHTML="<span>\u23F3 Syncing Homepage...</span>",c.style.color="var(--color-primary)",c.textContent="Updating homepage sections in ShopBase theme...";try{let p=await(await fetch("/api/sync-homepage",{method:"POST"})).json();p.success?(c.style.color="#2E7D32",c.textContent=`\u2705 ${p.message}`,d.innerHTML="<span>\u2713 Homepage Synced</span>"):(c.style.color="#D32F2F",c.textContent=`\u274C ${p.error||"Homepage sync failed"}`,d.innerHTML="<span>Retry Sync</span>",d.disabled=!1)}catch(n){c.style.color="#D32F2F",c.textContent=`\u274C Network Error: ${n.message}`,d.innerHTML="<span>Retry Sync</span>",d.disabled=!1}})}var I={USD:{symbol:"$",rate:1},EUR:{symbol:"\u20AC",rate:.92},GBP:{symbol:"\xA3",rate:.79},INR:{symbol:"\u20B9",rate:83.5},CAD:{symbol:"CA$",rate:1.36},AUD:{symbol:"AU$",rate:1.52}},k=class{constructor(){this.state={products:R.MOTHER_AND_BABY_PRODUCTS,activeCategory:"all",currency:"USD",searchQuery:"",sortBy:"featured",cart:JSON.parse(localStorage.getItem("corencore_cart")||"[]"),isCartOpen:!1,activeModalProduct:null,activePageKey:null,isAdminHubOpen:!1,appliedPromo:localStorage.getItem("corencore_promo")||null,quizStep:1,quizAnswers:{}},this.dom={},this.actions={setCurrency:this.setCurrency.bind(this),formatPrice:this.formatPrice.bind(this),setCategoryFilter:this.setCategoryFilter.bind(this),searchProducts:this.searchProducts.bind(this),setSortBy:this.setSortBy.bind(this),toggleCart:this.toggleCart.bind(this),addToCart:this.addToCart.bind(this),updateCartQuantity:this.updateCartQuantity.bind(this),removeFromCart:this.removeFromCart.bind(this),applyPromo:this.applyPromo.bind(this),proceedToCheckout:this.proceedToCheckout.bind(this),openProductModal:this.openProductModal.bind(this),closeProductModal:this.closeProductModal.bind(this),openPageModal:this.openPageModal.bind(this),closePageModal:this.closePageModal.bind(this),openAdminHub:this.openAdminHub.bind(this),closeAdminHub:this.closeAdminHub.bind(this)}}init(){console.log("\u{1F338} Core & Core Mother & Baby Storefront Loaded on Live ShopBase"),this.bindDom(),this.render()}bindDom(){this.dom={header:document.getElementById("header-container"),hero:document.getElementById("hero-container"),trustBar:document.getElementById("trust-bar-container"),categories:document.getElementById("categories-container"),routineQuiz:document.getElementById("routine-quiz-container"),productGrid:document.getElementById("product-grid-container"),safetyStandards:document.getElementById("safety-standards-container"),reviews:document.getElementById("reviews-container"),guideBanner:document.getElementById("guide-banner-container"),footer:document.getElementById("footer-container"),cartDrawer:document.getElementById("cart-drawer-root"),productModal:document.getElementById("product-modal-root"),pagesModal:document.getElementById("pages-modal-root"),adminHub:document.getElementById("admin-hub-root")}}setCurrency(e){this.state.currency=e,this.render()}formatPrice(e){let i=parseFloat(e)||0,r=I[this.state.currency]||I.USD,o=(i*r.rate).toFixed(2);return`${r.symbol}${o}`}setCategoryFilter(e){this.state.activeCategory=e,v(this.dom.productGrid,this.state,this.actions),f(this.dom.header,this.state,this.actions)}searchProducts(e){this.state.searchQuery=e,v(this.dom.productGrid,this.state,this.actions)}setSortBy(e){this.state.sortBy=e,v(this.dom.productGrid,this.state,this.actions)}toggleCart(e){this.state.isCartOpen=e,b(this.dom.cartDrawer,this.state,this.actions)}addToCart(e,i=1,r=null){var m,t;let o=r||((m=e.variants)==null?void 0:m[0])||{id:1,title:"Standard",price:e.price},l=`${e.id}-${o.id}`,h=this.state.cart.findIndex(s=>s.cartItemId===l);h>-1?this.state.cart[h].quantity+=i:this.state.cart.push({cartItemId:l,id:e.id,variantId:o.id,shopbaseHandle:e.shopbase_handle||e.handle,title:e.title,variantTitle:o.title!=="Standard"&&o.title!=="1 PC"?o.title:"",price:o.price||e.price,image:e.image||((t=e.images)==null?void 0:t[0]),quantity:i}),this.saveCart(),f(this.dom.header,this.state,this.actions),b(this.dom.cartDrawer,this.state,this.actions)}updateCartQuantity(e,i){let r=this.state.cart.find(o=>o.cartItemId===e);if(r&&(r.quantity+=i,r.quantity<=0)){this.removeFromCart(e);return}this.saveCart(),f(this.dom.header,this.state,this.actions),b(this.dom.cartDrawer,this.state,this.actions)}removeFromCart(e){this.state.cart=this.state.cart.filter(i=>i.cartItemId!==e),this.saveCart(),f(this.dom.header,this.state,this.actions),b(this.dom.cartDrawer,this.state,this.actions)}saveCart(){localStorage.setItem("corencore_cart",JSON.stringify(this.state.cart))}applyPromo(e){this.state.appliedPromo=e,localStorage.setItem("corencore_promo",e),b(this.dom.cartDrawer,this.state,this.actions)}proceedToCheckout(){if(!this.state.cart.length)return;let e=this.state.cart[0],i=e.shopbaseHandle||e.handle;i?window.location.href=`/products/${i}`:window.location.href="/collections/all"}openProductModal(e){this.state.activeModalProduct=e,C(this.dom.productModal,e,this.state,this.actions)}closeProductModal(){this.state.activeModalProduct=null,C(this.dom.productModal,null,this.state,this.actions)}openPageModal(e){this.state.activePageKey=e,_(this.dom.pagesModal,e,this.actions)}closePageModal(){this.state.activePageKey=null,_(this.dom.pagesModal,null,this.actions)}openAdminHub(){this.state.isAdminHubOpen=!0,B(this.dom.adminHub,this.state,this.actions)}closeAdminHub(){this.state.isAdminHubOpen=!1,B(this.dom.adminHub,this.state,this.actions)}render(){f(this.dom.header,this.state,this.actions),$(this.dom.hero,this.dom.trustBar,this.actions),A(this.dom.categories,this.actions),F(this.dom.routineQuiz,this.state,this.actions),v(this.dom.productGrid,this.state,this.actions),L(this.dom.safetyStandards),T(this.dom.reviews),H(this.dom.guideBanner,this.dom.footer,this.actions),b(this.dom.cartDrawer,this.state,this.actions),C(this.dom.productModal,this.state.activeModalProduct,this.state,this.actions),_(this.dom.pagesModal,this.state.activePageKey,this.actions),B(this.dom.adminHub,this.state,this.actions)}};function Y(){let a=window.location.pathname.toLowerCase();return a==="/"||a===""||a==="/index.html"}function P(){if(!Y())return;if(!document.getElementById("corencore-fonts")){let i=document.createElement("link");i.rel="preconnect",i.href="https://fonts.googleapis.com",document.head.appendChild(i);let r=document.createElement("link");r.rel="preconnect",r.href="https://fonts.gstatic.com",r.crossOrigin="anonymous",document.head.appendChild(r);let o=document.createElement("link");o.id="corencore-fonts",o.rel="stylesheet",o.href="https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,400..800;1,400..800&family=Plus+Jakarta+Sans:wght@300;400;500;600;700;800&display=swap",document.head.appendChild(o)}document.title="Core & Core \u2014 Pure Mother & Baby Care";let a=document.getElementById("corencore-storefront-root");if(!a){a=document.createElement("div"),a.id="corencore-storefront-root";let i=document.getElementById("app");i&&(i.style.display="none"),a.innerHTML=`
    ...
    `,document.body.appendChild(a)}new k().init()}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",P):P();return U(K);})();
